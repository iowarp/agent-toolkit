...
## Step 1: Get the Code
First, clone the `agent-toolkit` repository to your local machine using Git.

```bash
git clone https://github.com/iowarp/agent-toolkit.git
cd agent-toolkit
```
All subsequent commands should be run from the root of this `agent-toolkit` directory.

...
def main(server, branch, args):
    """Launch Agent Toolkit MCP servers with isolated dependencies using uvx"""
    ...
    if not server:
        ...
        for s in sorted(server_command_map.keys()):
            click.echo(f"  - {s}")
        click.echo("\nUsage: uvx agent-toolkit <server-name>")
        click.echo("   or: agent-toolkit <server-name> (if installed)")
        return
